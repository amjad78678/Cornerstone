...
const fs = require("fs");
const { sharpenImage } = require("../service/sharpImages");
...
const loadAdminHome = async (req, res) => {
  try {
    ...
    usersByYear = usersByYear.map((item) => ({
      _id: +item._id,
      count: item.count,
    }));
    ...
    const latestOrders = await Order.find()
      ...
      .populate("user_id")
      .limit(10);
    const firstOrders = await Order.find().sort({ createdAt: 1 });
    ...
    const pendingOrders = await Order.countDocuments({
      status: { $in: ["pending", "dispatched", "out for delivery", "placed"] },
    });
...
const listingCategory = async (req, res) => {
  try {
    ...
    const updateUser = await Category.findByIdAndUpdate(
      id,
      { is_Listed: true },
      { new: true }
    );
...
const postEditProduct = async (req, res) => {
  try {
    ...
    if (req.files) {
      const existingCount = (await Product.findById(req.body.id)).imageUrl
        .length;
      console.log("existCount" + existingCount);
      if (
        existingCount + req.files.length !== 5 ||
        existingCount + req.files.length > 5
      ) {
    ...
    if (status === "cancelled" || "returned") {
      ...
      for (let i = 0; i < orderData.items.length; i++) {
        ...
        await Product.updateOne(
          { _id: productId },
          { $inc: { stockQuantity: -productQuantity } }
        );
      }
    }
...
const loadSalesReport = async (req, res, next) => {
  try {
    ...
    res.render("salesReport", {
      moment,
      firstOrderDate: moment(firstOrder.createdAt).format("YYYY-MM-DD"),
      lastOrderDate: moment(lastOrder.createdAt).format("YYYY-MM-DD"),
      salesReport,
    });
  } catch (error) {
    console.log(error.message);
